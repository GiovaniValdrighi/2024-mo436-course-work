...
class TFCDataModule(L.LightningDataModule):
    def __init__(self, root_data_dir: Union[Path, str], batch_size: int = 32):
        self.root_data_dir = Path(root_data_dir)
        self.batch_size = batch_size

    def _get_dataset_dataloader(self, path: Path, shuffle: bool):
...
class TFCHarDataset(Dataset):
    def __init__(
        ...
        super(TFHarDataset, self).__init__()
        self.csv_file_path = Path(csv_file_path)
        ...
        self.x_data = torch.tensor(self.data[:, :, :])
        ...
        if (
            ...
            self.aug1 = DataTransform_TD(self.x_data)
